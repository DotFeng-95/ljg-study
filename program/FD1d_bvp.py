...
def FD1d_bvp(N, f, a, b, u):
    ...
    c1 = - 1 / (h ** 2)
    c2 = 2 / (h ** 2)
    g = np.hstack((c1 * np.ones(N - 2), np.array([0])))
    c = np.hstack(([np.array([0]), c1 * np.ones(N - 2)]))
    d = np.hstack((np.array([1]), c2 * np.ones(N - 2), np.array([1])))
...
x, U = FD1d_bvp(6, f, 0, 1, u)
u_exact = u(x)
e0, e1, emax = FD1d_error(x, U, u_exact)
