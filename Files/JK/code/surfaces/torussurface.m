function surfacedata = torussurface(RR, rr, nu, nv)
Nu = nu;
Nv = nv;
R = RR;
r = rr;
...
function [node, elem, UV]= initmesh()
...
function z = phi(p)
    z = sqrt(p(:,1).^2+p(:,2).^2+p(:,3).^2+R^2-2*R*sqrt(p(:,1).^2+p(:,2).^2))-r;
end


function n = gradient(p)
    s1=sqrt(p(:,1).^2+p(:,2).^2);
    s2=sqrt(s1.^2 + p(:,3).^2+R^2- 2*R*s1);
    n1 = -(R - s1).* p(:,1)./(s1.*s2);
    n2 = -(R - s1).* p(:,2)./(s1.*s2);
    n3 = p(:,3)./s2;
    n = [n1,n2,n3];
